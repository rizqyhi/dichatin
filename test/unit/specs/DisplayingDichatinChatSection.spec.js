...
describe('Displaying Dichatin chat section', () => {
  ...
  context('When chat provider was not throwing unexpected error', () => {
    it('should notn shown error notice', () => {
      const chatProvider = {
        isLogin: () => true,
        isError: () => false
      }
      ...
      expect(wrapper.find('.dichatin-chat__error').exists()).to.be.false()
    })
  })
})
